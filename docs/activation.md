...
To activate prediction results by directly uploading user lists to Google Ads Audience Manager, you'll need to ensure user IDs are recognizable by the Google Ads platform. This is achieved by setting up a Google Ads Manager tag.

### Prerequsite
Deploy the **Google Ads Remarketing Tag** via Google Tag Manager on your site. This tag collect user identifiers like `client id`, which is equivalent to `user_pseudo_id` in the GA4 export data. This collection is crucial for building audience lists in Google Ads. Fellow the detailed setup instruction under `Option 1: Set up the User ID parameter in your global site tag` from the official [Tag Manager documentation](https://support.google.com/google-ads/answer/9199250)

### Export and import instruction

1. Export Customer List from BigQuery
Export your customer list from the prediction tables generated by the MAJ solution by running the following query in BigQuery:
...
FROM (
  SELECT
    ...
    NTILE(10) OVER (ORDER BY prediction DESC) AS prediction_decile,
  FROM
    `{{source_table}}` )
WHERE
  prediction_decile IN ({{decile_list}})
```

Where:
* `source_table`: A reference to the actual BigQuery table containing prediction results (e.g., "purchase_propensity.predictions_2024_05_04T02_04_06_124Z_244_view").
* `decile_list`: A comma-separated list of decile values (e.g., "1,2" if you want to select the top 20% of the predicted users)

After running the query, export the result as a CSV file.

1. Upload Customer List to Google Ads

Next, follow the official Google Ads guide on [Create and upload your customer list](https://support.google.com/google-ads/answer/10589050?hl=en#zippy=%2Ccreate-and-upload-your-customer-list). When uploading, ensure you:
Make sure you
- Choose to `upload a plain text data file`
- Select `Data type` as `Upload User IDs`.

**NOTE:** It's important to regularly repeat the export and import steps as new visiting users are exported to BigQuery each day to ensure your marketing audience remains up-to-date.
